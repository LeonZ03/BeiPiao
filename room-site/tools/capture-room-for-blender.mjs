...
const site=path.resolve(import.meta.dirname,'..'),dist=path.join(site,'dist');
const out=path.resolve(site,'../generated-assets/full-room');fs.mkdirSync(out,{recursive:true});
...
const sourceFile=path.join(site,'tools/reference-scene.source.js');
...
function tex(t){if(textureIds.has(t))return textureIds.get(t);const id=textures.length;textureIds.set(t,id);
 ...
 const info={id,file,webPath:file.startsWith(dist)?'./'+path.relative(dist,file).replaceAll('\\','/'):null};
