...
7. **新房间先复用资产，再考虑新建。** 新房间不等于所有物件重做。建模前先检查现有房间中已获认可的物件、材质和交互；相同物件直接复用，有局部差异则复制一份修改，确实无合适基础时才从零建模。不能为了复用而把不同物件强行做成同一种样子。
...
- 开始逐件建模前，先完成下方的“跨房间资产复用”检查，并在新房间的约束记录中列出复用来源与需要修改的差异。
...
- **先找现成的。** 检查各房间 `assets/` 的独立源和当前完整 `.blend`，对照新参考确认型号、轮廓、比例及材质。独立物件源可能落后于完整场景里的后续精修，优先采用最终获认可的版本，不能只看文件名。头盔、电脑、杯子、高达等同一实物优先直接复用；柜子、床、卫浴等相似物件可作为修改基础。
- **直接复用。** 造型与材质一致时，通过官方 MCP 从源工程导入所需对象或集合，连同网格、材质、贴图、修改器、形态键、约束依赖和交互枢轴一起保留，只调整新房间中的摆放。重复物件可共享不可变网格/贴图；各实例的位置、开合、音效等状态必须独立。不要为复用重新运行旧房间整套生成脚本。
- **复制后修改。** 尺寸、颜色、结构或磨损不同，就在目标房间 `assets/<asset-id>/` 保存独立变体，再针对差异修改。编辑前将会改动的网格、材质、节点组和贴图等数据设为独立副本；仅复制对象但继续共用可写材质/网格不算隔离。避免非均匀拉伸破坏圆角、厚度、纹理比例或交互轴。
- **不回写来源。** 为新房间适配不得覆盖旧房间的源工程、已发布资产或共享数据。默认将对象追加到目标工程，不建立未经管理、会随源文件变化的跨房间链接。记录来源房间、文件、对象/集合名、Git 提交或资产版本、复用方式和差异；资产自身的材质、纹理、音效出处和许可信息也要保留。
- **按需提取公共资产。** 确认多个房间反复使用后，才整理到仓库根目录 `shared-assets/<asset-id>/`（需要时创建），保存可独立打开的源、必要贴图和简短说明，注明单位、原点、版本与使用方。不要为预设复用批量搬动当前房间；共享基线升级须显式选择使用方并回归，房间特有变体仍放各自 `assets/`。
- **适配后验收。** 核对米制尺度、坐标转换、接触面、材质与新光照的关系；只导入物件必要依赖，不带入原房间灯光、相机、投影代理或烘死的环境明暗。交互 refs/节点 ID 在目标房间重新映射，风动固定点、碰撞、开合范围和声音位置重新校准。最终检查网页近景与交互，并确认来源房间未被改变。
...
- 不显示“点击窗帘 · 拉开 / 合上”等悬停或瞄准操作提示；保留可点击物件的鼠标样式，以及动作触发后的状态反馈（窗帘开合、灯和水流开关、反锁等）。
